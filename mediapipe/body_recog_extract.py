...
def main():
    ...
    input_root_dir = sys.argv[1]  # 例如 technique space ...
    for subdir_name in os.listdir(input_root_dir):
        ...
        print(f"\n开始处理二级文件夹：{subdir_path}")
        
        # 查找当前二级文件夹下的所有.MOV文件
        mov_files = [f for f in os.listdir(subdir_path) 
                    if os.path.isfile(os.path.join(subdir_path, f)) 
                    and f.upper().endswith(".MOV")]  # 兼容大小写（如果有小写.mov也会处理）

        if not mov_files:
            print(f"  警告：{subdir_path} 中未找到.MOV文件，跳过")
            continue
        for mov_file in mov_files:
            input_video_path = os.path.join(subdir_path, mov_file)
            print(f"  处理文件：{input_video_path}")

            # 构建输出文件路径（与原文件同目录）
            video_basename = os.path.splitext(mov_file)[0]  # 去掉.MOV后缀
            output_video_path = os.path.join(subdir_path, f"{video_basename}_text.mp4")
            output_npy_path = os.path.join(subdir_path, f"{video_basename}_keypoints.pkl")
